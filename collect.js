/**
 * maimai DX 데이터 수집 북마클릿
 * 
 * 사용법:
 *   1. 아래 javascript: 로 시작하는 한 줄짜리 코드를 브라우저 북마크에 등록
 *   2. maimaidx-eng.com 로그인 후 북마크 클릭
 *   3. 업로드 창이 자동으로 열리고 데이터 수집 시작
 */
...
javascript:(function(){
  ...
  const typeFromEl = el => {
    ...
    if (src.includes('music_dx')) return 'DX';
    // 일부 페이지는 pointer 클래스로 구분
    if (el?.querySelector('img.music_kind_icon_dx')?.classList.contains('pointer')) return 'DX';
    return 'STANDARD';
  };
  ...
  // 달성률 문자열 → 정수 (100.5% → 1005000)
  const parseAch = str => {
    const n = parseFloat((str || '0').replace(/[^0-9.]/g, ''));
    return Math.round(n * 10000);
  };

  // DX 스코어 "1234 / 5678" → { score, max }
  const parseDx = str => {
  ...
  const collect = async () => {
    ...
    for (const diff of DIFFS) {
      ...
      for (const block of blocks) {
        ...
        const displayLevel = displayLevelFmt(form.querySelector('.music_lv_block')?.textContent?.trim());
        ...
        allRecords.push({
          track: { title, difficulty: diff, type, displayLevel, idx },
          score: { achievement, dxScore, combo, sync }
        });
      }
