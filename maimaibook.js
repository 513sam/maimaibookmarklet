...
if (sol) {
    document.getElementById('breakPerf').innerHTML = '<span class="count">' + sol['75%Perfect'] + '</span>-<span class="count">' + sol['50%Perfect'] + '</span>';
    document.getElementById('breakGreat').innerHTML = '<span class="count">' + sol['80%Great'] + '</span>-<span class="count">' + sol['60%Great'] + '</span>-<span class="count">' + sol['50%Great'] + '</span>';
}
...
function calcAll() {
    ...
    noteTypes.forEach(t => {
        const el = document.getElementById('ach_' + t);
        if (el) {
            const loss = getLoss(d.notes[t], weights[t]);
            el.innerHTML = '<b>-' + loss + '%</b>';
        }
    });
    const totalEl = document.getElementById('ach_total');
    if (totalEl) totalEl.innerHTML = '<b>' + totalPct.toFixed(4) + '%</b>';
    const finalEl = document.getElementById('finalRate');
    if (finalEl) finalEl.textContent = totalPct.toFixed(4) + '%';

    const totals = noteTypes.map(t => getTotal(d.notes[t]));
    const totalCountEl = document.getElementById('totalCounts');
    if (totalCountEl) totalCountEl.textContent = totals.join(' / ');
}
...
function updateCell(cell) {
    ...
    // 기존 화살표/실점 제거
    cell.querySelectorAll('.arrow, .loss').forEach(el => el.remove());
...
document.querySelectorAll('td.val').forEach(cell => {
    ...
    const jud = cell.dataset.j;
    if (!type || !jud) return;
    cell.dataset.orig = d.notes[type][jud];
    updateCell(cell);
});
